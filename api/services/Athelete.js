...
var model = {
    ...
    getOneAthlete: function (data, callback) {
        async.waterfall([
            function (callback) {
                Athelete.findOne({ _id: data._id }, function (err, complete) {
                    if (err) {
                        callback(err, null);
                    } else {
                        if (_.isEmpty(complete)) {
            ...
            function (response, callback) {
                ...
                Accounts.findOne({ athlete: response._id }).lean().deepPopulate("transaction,transaction.package").exec(function (err, found) {
                    if (err) {
                        callback(null, final);
                    } else {
                        if (_.isEmpty(found)) {
